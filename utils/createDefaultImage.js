...
const createDefaultImage = async () => {
  try {
    ...
    const dir = "./images";
    const temp = "./images/temp";
    const file = "./images/permanent";
    ...
    const existingUsers = await User.findOne({role: VAR.ADMIN});
    ...
    if (existingImages.length === 0) {
      ...
      const parFolderPath = path.join(file);

      const tempFilename = `${Date.now()}.jpg`;
      const parFilename = `${Date.now()}.jpg`;

      const tempImagePath = path.join(tempFolderPath, tempFilename);
      const parImagePath = path.join(parFolderPath, parFilename);

      fs.writeFileSync(tempImagePath, buffer);
      fs.writeFileSync(parImagePath, buffer);

      const defaultImageData = {
        name: `${parFilename}`,
        path: `/permanent/${parFilename}`,
        base64: base64Data,
        status: VAR.IMAGE_FILE,
        creator: existingUsers._id.toString(),
        updater: null,
      };

      const defaultImage = new Image(defaultImageData);
      await defaultImage.save();

      console.log("Default image created:", defaultImage);
    }
  } catch (error) {
